...
## 2026-09-07 — class、实例与状态

本轮评分：9/10。学习者准确解释 self、实例属性彼此独立，以及普通 class 不自动完成类型或范围校验；需要补准的是普通字典同样可以创建多份并同时存在，class 的优势不在于字典无法表示多份数据。

### 问题：AI 后端里，一次查询任务包含 table_name、limit，还需要生成 Tool payload 和修改 limit。为什么可能选择 QueryTask class，而不是两个全局变量或一个普通字典？请同时说明 self 的作用、两个实例的状态关系，以及普通 class 是否会自动验证 limit 的类型和范围。

**标准回答：**

两个全局变量只有一份共享状态，不适合同时表示多个查询任务，还会带来请求之间互相覆盖或污染的风险。普通字典可以创建多份并独立保存数据，适合 JSON 解析结果、临时记录或只有少量操作的松散结构；它并不是不能表示多个任务。

当“查询任务”是一个明确的领域概念，并且生成 payload、更新 limit 等行为与其状态紧密相关时，class 可以把状态和行为集中到一种对象类型中，便于复用和维护。self 表示当前正在操作的实例；task_a.update_limit(...) 中，task_a 会自动作为 self 传入，因此只修改 task_a 的实例属性，不影响 task_b。普通 Python class 不会自动验证 limit 是否为 int 或是否非负，相关规则必须显式编写；后续也可以使用 Pydantic 等工具建立更强的数据校验边界。

## 2026-09-08 — iterator、generator 与 yield

本轮评分：7/10。学习者准确覆盖峰值内存、generator 单次消费和 list 可重复遍历；主要缺口是误认为 generator 一定更快，没有把性能收益限定在上游也按需读取且调用方提前停止的条件下。

### 问题：RAG 服务需要从数据库读取 100 万个文档块，但一次任务找到前 100 个符合条件的块就会停止。为什么 generator 可能比先构造完整 list 更合适？它是否一定更快？如果同一批结果需要反复遍历三次，又应如何选择？

**标准回答：**

如果数据库游标和后续处理链都支持按需读取，generator 可以一次拉取并处理一条记录。调用方找到前 100 个匹配结果后停止时，剩余记录无需进入 Python 内存，也可能无需继续读取、解析或计算，因此通常能降低峰值内存，并节省提前停止后的工作。
